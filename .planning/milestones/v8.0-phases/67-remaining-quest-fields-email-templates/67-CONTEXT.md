...
### QuestLog Details mobile Rewards gap
- **D-03:** `Views/QuestLog/Details.Mobile.cshtml` has no Rewards display markup at all — confirmed by full-file read during pattern mapping (this contradicts this document's original Integration Points claim that it just needed a raw-to-`Html.Markdown()` swap; there is nothing there to swap). Desktop's `Views/QuestLog/Details.cshtml` does show Rewards. Same category of gap as D-02, same resolution: add the missing Rewards section to `Details.Mobile.cshtml` in this phase, mirroring desktop's layout (`.quest-description-box`, `fa-coins` icon, conditional on non-blank), rendered via `Html.Markdown()` from the start (no pre-wrap-removal step needed since the field never existed there as plain text).
...
- Exact markup for the new `QuestLog/Details.Mobile.cshtml` Rewards section (D-03) — mirror desktop `QuestLog/Details.cshtml`'s Rewards block placement/styling.
- **Pre-wrap CSS scoping (important — found during pattern mapping):** `.quest-description-box` and `.recap-display-box` in `quests.css` are shared classes — some occurrences render Rewards/Recap (in scope, converting to `Html.Markdown()`, pre-wrap should go) while others may still render plain-text content this phase does not touch. Do NOT strip `white-space: pre-wrap` at the shared class level in `quests.css` if that would affect an out-of-scope occurrence — scope the removal to an inline override on the specific in-scope elements instead, or verify (via grep) that no untouched field still depends on that class's pre-wrap before touching the shared rule.
...
- **Rewards read views:** `Views/Quest/Details.cshtml:38` (`.quest-description-box`, has inline `pre-wrap` to remove), `Details.Mobile.cshtml`, `Views/QuestLog/Details.cshtml:59` (`.quest-description-box`) — swap raw `@Model.Quest.Rewards` for `@Html.Markdown(Model.Quest.Rewards)`. `Views/QuestLog/Details.Mobile.cshtml` has no Rewards markup to swap — it needs the section added from scratch (D-03). Plus the new Manage.cshtml section (D-01).
