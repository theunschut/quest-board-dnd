# Requirements Archive: v5.0 Multi-Tenancy

**Status:** ✅ SHIPPED 2026-07-02
**Defined:** 2026-06-29
**Core Value:** The quest board must reliably let DMs post quests and players sign up — everything else enhances that loop.

All 48 requirements below shipped as planned; none were dropped or materially adjusted in scope. The GROUP-04/05/06 and TENANT-01/02/03/05 checkboxes were briefly stale (unchecked despite being complete per the traceability table) — corrected at milestone close.
...
- [x] **GROUP-01**: `GroupEntity` table exists with `Id`, `Name`, `CreatedAt` columns
- [x] **GROUP-02**: `UserGroups` junction table exists with `UserId`, `GroupId`, `GroupRole` (enum: Player / DungeonMaster / Admin)
- [x] **GROUP-03**: `GroupId` FK added to `QuestEntity` and `ShopItemEntity` (the two shared-resource entities not naturally scoped through user ownership)
- [x] **GROUP-04**: Data migration seeds the `"EuphoriaInn"` group as `GroupId = 1`
- [x] **GROUP-05**: All existing users are assigned to EuphoriaInn in `UserGroups`; each user's `GroupRole` is seeded from their current `AspNetUserRoles` entry
- [x] **GROUP-06**: `AspNetUserRoles` entries for Player / DungeonMaster / Admin are removed after migration; only SuperAdmin assignments remain in `AspNetUserRoles`

### Tenant Isolation

- [x] **TENANT-01**: `IActiveGroupContext` interface defined in the Domain layer with an `ActiveGroupId` property (`int?` — `null` means SuperAdmin, see all)
- [x] **TENANT-02**: `ActiveGroupContextService` in the Service layer implements `IActiveGroupContext`; reads `ActiveGroupId` from ASP.NET Core Session; returns `null` when the user holds the SuperAdmin role
- [x] **TENANT-03**: EF Core Global Query Filters applied to `QuestEntity` and `ShopItemEntity` using `IActiveGroupContext.ActiveGroupId`; `UserEntity` does NOT receive a query filter
...
- [x] **TENANT-05**: Integration test factory registers a stub `IActiveGroupContext` returning `GroupId = 1` by default; all 191 existing tests pass after filter addition
...
- [x] **MGMT-01**: A dedicated MVC Area exists for SuperAdmin group management (`/platform`, not `/superadmin`)
...
- [x] **REG-01**: Public self-registration (`AccountController.Register`) is removed
...
- [x] **SESSION-01**: ASP.NET Core Session is backed by `AddDistributedSqlServerCache` (package `Microsoft.Extensions.Caching.SqlServer` 10.0.9) registered BEFORE `AddSession` in `Program.cs`, so `ActiveGroupId` survives an app restart instead of being wiped from the in-memory fallback store. Registration is guarded by `if (!builder.Environment.IsEnvironment("Testing"))` with an `AddDistributedMemoryCache()` fallback in the Testing branch.
- [x] **SESSION-02**: The `dbo.AspNetSessionState` cache table is provisioned by an EF Core migration using `migrationBuilder.Sql(...)` with an `IF NOT EXISTS` guard; the `Id` column uses `COLLATE SQL_Latin1_General_CP1_CS_AS` (case-sensitive) per the official `dotnet/aspnetcore` DDL.

### Admin Email Rate Limiting (Phase 33)

- [x] **EMAIL-RATE-01**: `AdminController.SendConfirmationEmail` (POST) rejects the 4th request within 1 hour for the same target `userId` with HTTP 429, enforced via a programmatic `PartitionedRateLimiter<int>` `AttemptAcquire(userId)` check inside the action body.
...
- [x] **EMAIL-RATE-03**: `AdminController.EditUser` (POST) applies the same `AttemptAcquire(model.Id)` check only inside the email-dispatch sub-path, so non-email-changing saves are not counted.
...
- [x] **PWFLOW-01**: Admin-created accounts have no password at creation; `CreateUserViewModel.Password` and the password input in `CreateUser.cshtml`/`CreateUser.Mobile.cshtml` are removed
- [x] **PWFLOW-02**: A newly created user receives one combined "Welcome — set your password" email; clicking the link and submitting a new password sets `PasswordHash` AND marks `EmailConfirmed = true` in a single `SetPassword` action
- [x] **PWFLOW-03**: A passwordless account cannot sign in until `SetPassword` is completed — `PasswordSignInAsync` fails gracefully against a null `PasswordHash`
- [x] **PWFLOW-04**: Self-service "Forgot password?" flow sends a reset link that lands on `SetPassword`; the POST is enumeration-safe and rate-limited to 3 requests / 15 minutes per client IP
- [x] **PWFLOW-05**: The admin "Resend welcome email" button sends the new Welcome email; the old `ConfirmEmail.razor` + `ConfirmationEmailJob` flow is retired
- [x] **PWFLOW-06**: `DataProtectionTokenProviderOptions.TokenLifespan` is configured to 7 days

## Future Requirements

Not addressed in v5.0 — carried forward.

- **"Password changed" notification email** — for `AccountController.ChangePassword` (self-service) and `AdminController.ResetPassword` (admin-triggered); deferred from Phase 32 to a future phase
...
- **Email non-uniqueness / separate account per group** — one global user identity with per-group roles
...
| GROUP-01 | Phase 27 | Complete |
| GROUP-02 | Phase 27 | Complete |
| GROUP-03 | Phase 27 | Complete |
