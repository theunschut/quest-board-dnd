...
- [x] 46-07-PLAN.md — Real-device verification, gated on an explicit device-access confirmation decision (Pre-Execution Blocker): desktop end-to-end crop/dual-file/D-03 flow, then iOS Safari EXIF orientation, 12MP canvas-memory ceiling, and touch-drag precision on a real device. Deferred at phase close for lack of device access; completed by the user on a real iPhone before v7.0 milestone close (2026-07-08) — all checks passed (`46-UAT.md`).
...
### Phase 48: Open Board Action on Platform Group Index
...
### Phase 55: Fix cross-tenant quest leak on quest board
...
### Phase 58: Rename the Guild Members feature to Characters everywhere
...
### Phase 59: Add a rewards field to quests
...
### Phase 60: Stop creating AspNetUserRoles entries for new users
...
**Key Decisions:** See `.planning/PROJECT.md` Key Decisions table for the full list — v7.0 added ~20 rows spanning Phases 43–64, including the `body::before` fixed-background pattern (Phase 43), authorization-must-validate-target-group lesson (Phase 49), the SuperAdmin-sees-exactly-what-a-user-sees governing principle (Phase 55), and mobile-parity-pairs-tasks-not-phases (Phase 61).

**Issues Resolved:** Two real cross-tenant security leaks found and closed mid-milestone — CharacterEntity/DM-profile/PlayerSignup group-tenant filtering gaps (Phase 49) and a SuperAdmin null-ActiveGroupId escape hatch plus a SelectGroup IDOR gap (Phase 55). Also closed: a silent no-email gap on Platform group-add (Phase 47), a Campaign-quest edit-page parity gap (Phase 50), a Dead-character server-side revival gap (Phase 52), a critical character-edit admin-impersonation bug (Phase 56), and a critical hidden-Contact image-visibility bypass (Phase 57).

**Issues Deferred:** Phase 46's real-device touch/EXIF/iOS-canvas-memory verification was deferred at phase close (device access unavailable) and completed by the user before milestone close — no longer open. Digest batching for session reminders (EMAIL-04/REMIND-02) and the `GroupSessionMiddleware` POST data-loss risk (flagged Phase 31) both remain open, carried into the next milestone.

**Technical Debt Incurred:** A pre-existing dead-code bug in `GuildMembersController.Edit`'s `SetAsMainCharacterAsync` demotion guard was found (not introduced) during Phase 56 verification and spun off as a separate follow-up task, not fixed in-phase.
