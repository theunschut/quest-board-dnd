...
### Existing data
- **D-03:** Leave existing stale `AspNetUserRoles` rows alone. Do **not** write a data-cleanup migration. Historic "Player"/"DungeonMaster"/"Admin" rows (seeded originally by the `20250704211037_ConvertIsDungeonMasterToRoles` migration, then added to by every subsequent `CreateUserAsync` call since) stay in the database as harmless, unread cruft. This phase only stops new rows from being written going forward — no schema/data migration in this phase.
...
- `QuestBoard.Repository/Migrations/20250704211037_ConvertIsDungeonMasterToRoles.cs` — the original 2025 migration that seeded the now-defunct `Player`/`DungeonMaster`/`Admin` `AspNetRoles` rows; left untouched per D-03.
