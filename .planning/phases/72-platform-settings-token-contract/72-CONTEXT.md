...
SuperAdmin-only Platform Settings page for the Omphalos integration (URL, shared secret, enabled toggle), backed by a single-row `IntegrationSettingEntity` and an EF Core migration — plus the written HMAC token-format contract that Phases 73 (Quest Board) and 74 (Omphalos) will both implement against. This phase writes the contract down; it does not implement token generation or the SSO endpoint themselves (those are Phase 73/74).
...
### Requirements & Scope
- `.planning/REQUIREMENTS.md` — SETT-01 through SETT-08 (locked requirements for this phase); also TOKEN-01/02/03/04/06 (token contract fields this phase must write down, even though the code implementing them lands in Phase 73)
