...
- **D-01 (RESOLVED as of this sanity check):** Phase 62 was sequenced behind Phase 45 (45-02, 45-03) and Phase 46 because those phases touched the exact files/methods this phase needs to change. **Both have now fully executed** (every sub-plan has a SUMMARY.md). Verified directly against current code:
...
- **Consequence for planning:** the write-path safety net (formerly D-03/D-04's concern) is **fully built and in production already**. Phase 62's planner does **not** need to touch `CharacterService`/`ContactService`/`ContactRepository`/`CharacterRepository`'s update paths or the picture-read methods at all — only the six *list/detail read* methods above need the `.Include(...ProfileImage)` removed and a `HasProfilePicture`/`HasContactImage` projection added in its place.
...
- **D-03 (RESOLVED):** Naively removing `.Include(x => x.ProfileImage)` from the read-side methods without also fixing the write path would have reproduced a real bug: `CharacterService.UpdateAsync`/`ContactService.UpdateAsync` used to unconditionally call `repository.UpdateProfileImageAsync(model.Id, model.ProfilePicture, token)` — passing `null` wiped the stored image. Phase 45-02 fixed this with the `hasNewOriginalUpload` signal described in D-01. No longer a live concern for Phase 62.
- **D-04 (confirmed, unchanged):** `DungeonMasterProfileService.UpsertProfileAsync` (lines ~17-46) is **still the correct reference pattern** — it only touches the image when the caller explicitly passes new bytes or `removeImage: true`, never relying on a round-tripped read value. `GetProfileByUserIdAsync`'s eager-load remains pure waste with zero write-path risk — safe to fix.
