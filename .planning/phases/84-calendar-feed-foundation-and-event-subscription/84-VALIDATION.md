...
| `84-04-T5` | `84-04` | 2 | CALFEED-17 | T-84-04 | A tombstone retired longer ago than the configured retention window is purged and its address then answers 404; one inside the window, and a subscription never retired, are left alone | unit (repository, boundary) | `dotnet test QuestBoard.UnitTests --filter CalendarSubscriptionRepositoryTests` | ❌ W0 | ⬜ pending |
