# Phase 65: Platform Settings + Token Contract - Discussion Log
...
|--------|-------------|----------|
| Explicit nonce field (recommended) | Add a random token-ID field to the canonical message; Phase 67 keys its used-token table on it | ✓ |
| Hash of the full token | Phase 67 stores a hash of the complete signed token as its "used" key | |
...
| Keep it in Quest Board's .planning/, reference it from the Phase 67 PR | No new file in Omphalos; PR description links back | ✓ |
| You decide | Let Claude pick based on friction | |

**User's choice:** Keep it in Quest Board's .planning/, reference it from the Phase 67 PR
