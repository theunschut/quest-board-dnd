...
- [ ] 34.3-01-PLAN.md — shared `GetEffectiveGroupRoleAsync` helper on `IUserService`/`UserService` (SuperAdmin bypass + group-role lookup, D-02) + unit tests
...
- [ ] 34.3-02-PLAN.md — `QuestController` 12 sites migrated (Index/Details `CanManage`/Manage `IsAdmin` + 8 `Forbid()` ownership-override guards; D-03 fail-hard)
