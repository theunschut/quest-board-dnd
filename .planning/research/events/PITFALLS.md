...
4. Treat the row-growth number as a real design input, not a hypothetical: with 2-3 concurrent weekly series across the board's membership, expect low-thousands of signup rows within the first year. This is fine for SQL Server at this scale, but it means the overview page (Pitfall 8) cannot afford to load the full 12-month window per render.
