...
| verification | Phase 87 group-picker skip path from a real non-SuperAdmin account | Deferred — the branch is gated on `!isSuperAdmin` and so is structurally unreachable from the operator own account; accepted on 9 integration facts with non-SuperAdmin test clients instead of a live browser check | v9.0, Phase 87 |
